...
class WorldBoard:
    def __init__(self, board_size: Tuple[float] = (100, 100)):
    ...
    def spawn(self, entity_type: str, number_to_spawn: int = 100):
        for i in range(number_to_spawn):
            ...
            point, =  self.ax.plot([entity.position[0]], [entity.position[1]], 'o', color=entity.colour)
            entity.point = point
    ...
    def step(self):
        ...
        if self.day % 100 == 0:
            self.spawn_plants(number_to_spawn=5)
        ...
        def update_plot(n):
            ...
            for entity in self.object_list:
                ...
                if entity.show:
                    ...
                    point.set_data([-1, -1])
            if self.day % 10 == 0:
                print(f"day: {self.day}, time: {time.time() - stationary_time}, average: {1000 * ((time.time() - stationary_time) / self.day):.2f}ms")

        ani = animation.FuncAnimation(self.fig, update_plot, interval=5)
        plt.show()
