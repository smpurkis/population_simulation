...
def distance_between_points_vectorized(entity_position: np.ndarray, positions: np.ndarray,
                                       board_size: np.ndarray) -> np.ndarray:
    # pythran export distance_between_points_vectorized(float [], float [], float [])
    abs_diff = np.abs(positions - entity_position)

    x_distance = np.amin(np.array([abs_diff[:, 0], board_size[0] - abs_diff[:, 0]]), axis=0)
    y_distance = np.amin(np.array([abs_diff[:, 1], board_size[1] - abs_diff[:, 1]]), axis=0)
...
class WorldArea:
    def __init__(
            self, area_radius: int, entities: List[BaseEntity], position: np.ndarray, board_size: np.ndarray
    ):
    ...
    def set_entities_in_radius_vec(self, entities: List[BaseEntity]) -> List[BaseEntity]:
        positions = np.array([e.position for e in entities])
        distances = distance_between_points_vectorized(self.position, positions, self.board_size)
        ...
        for index, dist_ent in enumerate(distances_entities):
            if dist_ent[0] > self.area_radius:
                cut_off_index = index
                break
        entities_in_radius = [dist_ent[1] for dist_ent in distances_entities[:cut_off_index]]
        return entities_in_radius

    def update_entities_in_radius(self, showing_entities: List[BaseEntity]) -> List[BaseEntity]:
        entities_in_radius: List[BaseEntity] = []
    ...
    def update(self, entities: List[BaseEntity], showing_entities: List[BaseEntity], step_no: int):
        if step_no % 1 == 0:
